...
def make_traversals(grid, halo, n_threads, left_first):
    return Traversals(
        grid=grid, halo=halo, jit_flags=jit_flags, n_threads=n_threads, left_first=left_first
    )
...
class TestTraversals:
    ...
    def test_apply_scalar(
        ...
        if len(grid) == 1 and n_threads_ > 1:
            return
        cmn = make_commons(grid, halo, n_threads_, left_first)
    ...
    def test_apply_vector(n_threads_, halo: int, grid: tuple, left_first: bool = True):
        if len(grid) == 1 and n_threads_ > 1:
            return
        cmn = make_commons(grid, halo, n_threads_, left_first)
